...
module.exports.commentController = {
    getCommentsForPost: async (req, res) => {
        try {
          const postId = req.params.postId; // Обратите внимание, что здесь postId должно быть таким же, как в роуте ("/post/:postId/comments")
    
          // Добавьте проверку, что postId является корректным ObjectId
  ...
  addComment: async (req, res) => {
    const { text } = req.body;
    const { postId } = req.params; // Предположим, что postId передается в URL
